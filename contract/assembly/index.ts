...
var vote1 = 0;
var vote2 = 0;
var user = new Map<string,bool>();

storage.set("v1","0")


export function get1():number{
  return parseInt(storage.getSome("v1"));
}

export function Vote1():void{
//  if(!user.has(Context.sender)){
    vote1= parseInt(storage.getSome("v1"))
    vote1++;
    storage.set("v1",vote1)
    //++a a++ a+=1 <=> a=a+1
    //user.set(Context.sender,true)
  //}
    logging.log(parseInt(storage.getSome("v1")))

}

export function get2():number{
  return vote1;
}
